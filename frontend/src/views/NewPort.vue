<template>
  <div class="viewport">
    <h1>New Port:</h1>
    <el-row type="flex" class="form-row" justify="center">
      <el-col :span="6">
        <el-form ref="form" class="form" :model="form" :rules="formValidators" label-position="right" @submit.native.prevent>
          <el-form-item label="Port Name" prop="name">
            <el-input class="port-name-input" placeholder="Example: Hong Kong" v-model="form.name" @keyup.enter.native="onSubmit"></el-input>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="onSubmit">Create</el-button>
            <el-button @click="onCancel">Cancel</el-button>
          </el-form-item>
        </el-form>
      </el-col>
    </el-row>
  </div>
</template>

<script>
export default {
  data () {
    return {
      form: {
        name: ''
      },
      formValidators: {
        name: [
          { required: true, message: 'Port name cannot be empty', trigger: 'blur' }
        ]
      }
    }
  },
  methods: {
    onSubmit () {
      this.$refs.form.validate((isValid, errors) => {
        if (!isValid) {
          return Object.entries(errors).forEach(entry => {
            setTimeout(() => this.$notify.error({ title: 'Error', message: entry.pop().pop().message }), 50)
          })
        }

        const params = { name: this.form.name }
        this.$services.port.create(params).then(() => this.$notify.success({ title: 'Success', message: 'Port created successfully!' }))
        this.onCancel()
      })
    },
    onCancel () {
      this.$refs.form.resetFields()
    }
  }
}
</script>

<style lang="scss" scoped>
div.viewport {
  h1 {
    text-align: center;
    margin-bottom: 20px;
  }
}

.form {
  border: 1px solid #eee;
  border-radius: 10px;
  padding: 15px 15px 0px 15px;
  .port-name-input {
    width: 70%;
  }
}
</style>
